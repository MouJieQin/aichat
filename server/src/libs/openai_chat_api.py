...
class OpenAIChatAPI:
    ...
    def _create_system_prompt(
        ...
        你的回复使用的语言种类要参考上下文对话内容和用户设置(本条消息使用了中文，但不作为上下文语种参考依据，title、suggestions要和response的使用相同的语种。)，并且严格按照以下格式进行回复（请严格保持此纯json格式，不要添加额外文本,不要把回复内容包裹在```json```中）：
        {{
    ...
    async def _chat_imple(
        ...
        prompt_messages.append(
            {
                "role": "system",
                "content": "请注意回复格式要严格遵循系统设置的纯json格式，但不要使用markdown的```json```格式，并且使用的语言种类要参考上下文对话内容。",
            }
        )
